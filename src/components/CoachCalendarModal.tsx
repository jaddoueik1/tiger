                      ...
                      `}
                                        >
                                            ...
                                            <div className="mt-1 space-y-1">
                                                {hasPublic && (
                                                    <div className={`text-xs px-1 py-0.5 rounded ${isSelected ? 'bg-white/20' : 'bg-accent/20 text-accent'
                                                        }`}>
                                                        Class
                                                    </div>
                                                )}
                                                {hasPrivate && (
                                                    <div className={`text-xs px-1 py-0.5 rounded ${isSelected ? 'bg-white/20' : 'bg-primary/20 text-primary'
                                                        }`}>
                                                        Private
                                                    </div>
                                                )}
                                                ...
                                                >
                                                    <div className="flex items-start justify-between mb-2">
                                                        <h5 className="font-medium text-text">
                                                            {session.isPrivate ? 'Private Class' : session.name || 'Group Class'}
                                                        </h5>
                                                        <span className={`text-xs px-2 py-1 rounded-full ${session.isPrivate
                                                            ? 'bg-primary/10 text-primary'
                                                            : 'bg-accent/10 text-accent'
                                                            }`}>
                                                            {session.isPrivate ? 'Private Class' : 'Group'}
                                                        </span>
                                                    </div>
