...
import { useCoach, useCoachSessions, usePrivateSessionBooking, useWhatsAppOrder } from '@/hooks/useApi';
import { useAuthStore } from '@/store/authStore';
import { addDays, addMonths, format, isAfter, isSameDay, parseISO, startOfWeek } from 'date-fns';
...
export default function CoachProfilePage() {
    ...
    // Get coach booked sessions (instances) - Fetching next 3 months
    const [now] = useState(() => new Date());
    const [threeMonthsLater] = useState(() => addMonths(new Date(), 3));
    const { data: sessionsData, isLoading: sessionsLoading } = useCoachSessions(coachId, now, threeMonthsLater);
    ...
    const upcomingSessions = bookedSessions.filter((session: any) =>
        isAfter(parseISO(session.sessionDate), yesterday)
    ).sort((a: any, b: any) =>
