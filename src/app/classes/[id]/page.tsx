...
import { useClassSessions, useCoach, useClassTemplateById } from '@/hooks/useApi';
...
export default function ClassDetailPage() {
    ...
    const { data: classData, isLoading: classLoading } = useClassTemplateById(classId);
                                ...
                                <div className="relative h-64 md:h-80 rounded-2xl overflow-hidden mb-6">
                                    ...
                                    <div className="absolute bottom-6 left-6 right-6">
                                        <div className="flex items-center space-x-3 mb-3">
                                            <span className={`px-3 py-1 rounded-full text-sm font-medium ${difficultyColors[classTemplate.level as keyof typeof difficultyColors] || difficultyColors.all_levels
                                                }`}>
                        ...
                        <div className="space-y-6">
                            ...
                            >
                                ...
                                <div className="space-y-3">
                                    {classTemplate.prices ? (
                                        <>
                                            <div className="flex justify-between items-center">
                                                <span className="text-text-muted">Drop-in</span>
                                                <span className="font-semibold text-text">
                                                    {classTemplate.prices.dropIn ? `$${classTemplate.prices.dropIn}` : 'Free'}
                                                </span>
                                            </div>
                                            {(classTemplate.prices.pack10 || 0) > 0 && (
                                                <div className="flex justify-between items-center">
                                                    <span className="text-text-muted">Class Pack (10)</span>
                                                    <span className="font-semibold text-text">${classTemplate.prices.pack10}</span>
                                                </div>
                                            )}
                                            {(classTemplate.prices.monthly || 0) > 0 && (
                                                <div className="flex justify-between items-center">
                                                    <span className="text-text-muted">Monthly</span>
                                                    <span className="font-semibold text-text">${classTemplate.prices.monthly}</span>
                                                </div>
                                            )}
                                            {(classTemplate.prices.unlimited || 0) > 0 && (
                                                <div className="flex justify-between items-center">
                                                    <span className="text-text-muted">Unlimited</span>
                                                    <span className="font-semibold text-text">${classTemplate.prices.unlimited}</span>
                                                </div>
                                            )}
                                        </>
                                    ) : (
